...
declare namespace browser.bookmarks {
  const getChildren: (id: string) => Promise<chrome.bookmarks.BookmarkTreeNode>;
}

declare namespace browser.history {
  const search: (query: chrome.history.HistoryQuery) => Promise<chrome.history.HistoryItem>;
}

declare namespace browser.cookies {
  const remove: (details: chrome.cookies.Details) => Promise<chrome.cookies.Cookie>;
  const get: (details: chrome.cookies.Details) => Promise<chrome.cookies.Cookie>;
}
